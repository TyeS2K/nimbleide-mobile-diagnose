...
function boot() {
	...
	$nim.Load32 = {
		...
		ArrayLoad: function(list, index) {
			...
			if (doc.getElementById(item[list][index].id) == null) {
				...
				if (!item[list][index].done) {
					item[list][index].done = true; 
					if (self._route(item,[list,index,'call'])) {
						setTimeout(function() {						
							if (self._route(item,[list,index,'call'])) item[list][index].call(list);
						}, 80);
					}
					...
					setTimeout(function() {
						$nim.Load32.reTry++;
						if ($nim.Load32.reTry < 5) {
							$nim.Load32.ArrayLoad(list, index);
						}
					}, 2000);
				}
			}
	 ...
	 **/
	var thread = $nim.Load32.Clear(true);
	...
	$nim.Load32.Add ('j32-load', ($nim.Agent.isMobile ? 'js/mobile.' : 'js/') + 'jsloader.js', 'js');
	($nim.Load32.list[thread].length > 0) ? $nim.Load32.ArrayLoad(thread, 0) : $nim.Load32.Boot_Complete();
}
